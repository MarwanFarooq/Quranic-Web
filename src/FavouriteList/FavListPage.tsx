    ...
    <div className="bg-black">
      <Grid templateColumns="repeat(5, 1fr)">
        ...
        <GridItem id="backgroundcolor" colSpan={4} p={0}>
          <GridItem
            id=""
            className="favlistPage shadow-lg shadow-[#8778BC]"
            p={10}
          // colSpan={4}
          >
            <div className="flex gap-5 items-center flex-wrap justify-center">
              <div className="favlist  p-10  flex justify-center items-center cursor-pointer  ">
                <FaHeart className="text-white text-5xl " />
              </div>
              <div className="flex flex-col items-end gap-2 ">
                <h1 className="text-white laptop:text-6xl tablet:text-4xl mobile:text-2xl font-bold ">
                  {language === "ar" ? "القراء المفضلين" : "Liked Reciters"}
                </h1>
                <p className="text-gray-300">
                  <span className="ms-3" >
                    {userobjecttype?.favList.length}
                  </span>                  {language === "ar" ? "القراء" : "Reciters"}
                </p>
              </div>
            </div>
          </GridItem>
          <div className="flex justify-between flex-wrap ">
            <div className="flex items-center gap-3 p-5 flex-wrap"></div>
          </div>
          {false ? (
            ...
            //  grid desktop:grid-cols-4 laptop:grid-cols-3 tablet:grid-cols-2 tablet:mx-7 tablet:gap-6 mobile:grid-cols-2 1mobile:gap-0 !mobile:mx-0  p-4 
            <Grid className="tablet:grid tablet:grid-cols-6 mobile:flex mobile:justify-center ">
                    ...
                    <div className="tablet:p-5 mobile:p-3  transition ease-linear  !cursor-pointer group hover:bg-[#58545485]  ">
                      <WrapItem className="group " position={"relative"}>
                        <Avatar
                          onClick={() => {
                            navigator(
                              `/home/ReciterisDetails/${currentreciters.id}`
                            );
